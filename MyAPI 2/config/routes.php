...
function uri_segment($idx = 0){
	...
	if(isset($segments[$idx]))return $segments[$idx] ; else return '';
}
...
function getController(){
	...
	if(file_exists($c1)){
		$method = uri_segment(3) == '' ? 'index' : uri_segment(3); $params = uri_segment(4);
		//echo $method.'<br>'.$params;exit;
		InController($c1, $controlername, $method, $params);
	}elseif(file_exists($c2)){
		$method = uri_segment(4) == '' ? 'index' : uri_segment(4); $params = uri_segment(5);
		//echo $method.'<br>'.$params;exit;
		InController($c2, uri_segment(3), $method, $params);
	}elseif(file_exists($c3)){
		$method = uri_segment(2) == '' ? 'index' : uri_segment(2); $params = uri_segment(3);
